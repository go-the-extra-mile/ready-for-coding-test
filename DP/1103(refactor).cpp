...
int cache[50][50]; // cache[y][x] = (y, x)에서 움직일 수 있는 최대 횟수(부분문제)
...
int dfs(int y, int x) {
    ...
    visit[y][x] = true;
    int mx_moves = 1; // minimum number of moves(when no move possible) = 1
    for (int dir = 0; dir < 4; dir++) {
        int ny = y + dy[dir] * board[y][x];
        int nx = x + dx[dir] * board[y][x];

        if (0 <= ny && ny < n && 0 <= nx && nx < m) { // in board
            mx_moves = max(mx_moves, dfs(ny, nx) + 1);
        }
    }
    visit[y][x] = false;
    
    return cache[y][x] = mx_moves; 
}
